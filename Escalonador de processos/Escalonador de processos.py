...
class RoundRobin(ProcessScheduleSimulator):
    def simulate(self, nome_arquivo):
        ...
        while self.total_exec_time(process_list):
            ...
            for item in process_list:
                if item.tempo_chegada <= current_time and item.finalizado == False:
                    if not previous or previous.processo != item.processo:
    ...
    def queue(self, temporary_queue, process_queue, current_time):
        ...
        time = 0
        if temporary_queue:
            process_queue.append(temporary_queue[0])
            if temporary_queue[0].tempo_execucao <= 5:
                time += temporary_queue[0].tempo_execucao
                temporary_queue[0].finalizado = True
                temporary_queue[0].tempo_execucao = 0
            else:
                time += 5
                previous = temporary_queue[0]
                temporary_queue[0].tempo_execucao -= 5
